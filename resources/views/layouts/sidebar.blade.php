...
<div id="sidebar" class="sidebar-transition w-64 bg-gradient-to-b from-gray-900 to-gray-800 text-white flex-shrink-0 lg:block hidden flex flex-col">
    ...
    <nav class="px-4 space-y-2 mt-6 flex-1 overflow-y-auto">
        ...
        <a href="{{ route('hearings.index') }}" class="nav-item {{ request()->routeIs('hearings.index') ? 'active' : '' }} flex items-center space-x-3 px-4 py-3 rounded-lg">
        ...
        <a href="{{ route('documents.index') }}" class="nav-item {{ request()->routeIs('documents.index') ? 'active' : '' }} flex items-center space-x-3 px-4 py-3 rounded-lg">
        ...
        <a href="{{ route('reports.index') }}" class="nav-item {{ request()->routeIs('reports.index') ? 'active' : '' }} flex items-center space-x-3 px-4 py-3 rounded-lg">
        ...
        @auth
            ...
            @endif
            <div class="px-4 mt-6 mb-2">
                <p class="text-xs uppercase tracking-wider text-gray-400 font-semibold">AI Assistant</p>
            </div>

            <a href="{{ route('ai.index') }}" class="nav-item {{ request()->routeIs('ai.*') ? 'active' : '' }} flex items-center space-x-3 px-4 py-3 rounded-lg">
...
<div id="mobileSidebar" class="mobile-sidebar fixed inset-0 z-50 lg:hidden">
    ...
    <div class="relative w-64 h-full bg-gradient-to-b from-gray-900 to-gray-800 text-white flex flex-col">
        ...
        <nav class="px-4 space-y-2 mt-6 flex-1 overflow-y-auto">
            ...
            @if(auth()->user()->hasPermission('hearings.view'))
            <a href="{{ route('hearings.index') }}" onclick="toggleMobileSidebar()" class="nav-item {{ request()->routeIs('hearings.index') ? 'active' : '' }} flex items-center space-x-3 px-4 py-3 rounded-lg">
                <i class="fas fa-calendar-alt w-5 text-center"></i>
                <span>Hearings</span>
            </a>
            @endif
            ...
            @if(auth()->user()->hasPermission('documents.view'))
            <a href="{{ route('documents.index') }}" onclick="toggleMobileSidebar()" class="nav-item {{ request()->routeIs('documents.index') ? 'active' : '' }} flex items-center space-x-3 px-4 py-3 rounded-lg">
                <i class="fas fa-file-alt w-5 text-center"></i>
                <span>Documents</span>
            </a>
            @endif
            ...
            @if(auth()->user()->hasPermission('reports.view'))
            <a href="{{ route('reports.index') }}" onclick="toggleMobileSidebar()" class="nav-item {{ request()->routeIs('reports.index') ? 'active' : '' }} flex items-center space-x-3 px-4 py-3 rounded-lg">
                <i class="fas fa-chart-bar w-5 text-center"></i>
                <span>Reports</span>
            </a>
            @endif
...
<div class="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 z-50">
    <div class="flex justify-around py-2">
        ...
        @if(auth()->user()->hasPermission('documents.view'))
        <a href="{{ route('documents.index') }}" class="flex flex-col items-center p-2 rounded-lg transition-colors {{ request()->routeIs('documents.index') ? 'text-blue-600' : '' }}">

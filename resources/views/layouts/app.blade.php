...
<script>
document.addEventListener('DOMContentLoaded', function() {
    const askAiButton = document.querySelector('button:contains("Ask AI")');
    if (askAiButton) {
        askAiButton.addEventListener('click', function() {
            ...
            overlay.innerHTML = `
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl" style="height: 80vh;">
            ...
            // Load AI content via AJAX
            fetch('/ai/status')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        // Load the AI component
                        const aiContainer = overlay.querySelector('.p-4');
                        aiContainer.innerHTML = `
                            <div id="aiChatContainer"></div>
                        `;
                        
                        // You can load the full AI component here
                        // Or fetch it via AJAX and insert it
                    } else {
                        overlay.querySelector('.p-4').innerHTML = `
                            <div class="text-center py-8">
                                <i class="fas fa-exclamation-triangle text-3xl text-yellow-600 mb-4"></i>
                                <p class="text-gray-600 dark:text-gray-400 mb-2">AI Assistant Offline</p>
                                <p class="text-sm text-gray-500">Please start Ollama server to use AI features</p>
                            </div>
                        `;
                    }
                });
